...
public class Bishop implements Piece{
    ...
    public Bishop(int c, int[] initPos, boolean promoted){
        colour = c;
        pos = initPos; 
        pieceName = "bishop";
    ...
    public ArrayList getAllMoves(Board board, boolean getCoverage){
        ...
        int row = pos[0] + 1;
        int col = pos[1] + 1;        
        while(row < 8 && col < 8 && (getCoverage || (!getCoverage && (board.getPieceOnSquare(row, col) == null ||
                board.getPieceOnSquare(row, col).getColour() != this.colour)))){
            legalMoves.add(new Integer[]{row, col});
            //if the move has this piece capture an opponent's piece, stop checking along this diagonal
        ...
        row = pos[0] - 1;
        col = pos[1] - 1;        
        while(row >= 0 && col >= 0 && (getCoverage || (!getCoverage && (board.getPieceOnSquare(row, col) == null ||
                board.getPieceOnSquare(row, col).getColour() != this.colour)))){
            legalMoves.add(new Integer[]{row, col}); 
            //if the move has this piece capture an opponent's piece, stop checking along this diagonal
            if(board.getPieceOnSquare(row, col) != null && 
        ...
        row = pos[0] - 1;
        col = pos[1] + 1;        
        while(row >= 0 && col < 8 && (getCoverage || (!getCoverage && (board.getPieceOnSquare(row, col) == null ||
                board.getPieceOnSquare(row, col).getColour() != this.colour)))){
            legalMoves.add(new Integer[]{row, col});
            //if the move has this piece capture an opponent's piece, stop checking along this diagonal
            if(board.getPieceOnSquare(row, col) != null && 
        ...
        row = pos[0] + 1;
        col = pos[1] - 1;        
        while(row < 8 && col >= 0 && (getCoverage || (!getCoverage && (board.getPieceOnSquare(row, col) == null ||
                board.getPieceOnSquare(row, col).getColour() != this.colour)))){
            legalMoves.add(new Integer[]{row, col});
            //if the move has this piece capture an opponent's piece, stop checking along this diagonal
            if(board.getPieceOnSquare(row, col) != null && 
    ...
    public void setSquareTableValues(){
        ...
        try{
            Scanner input = new Scanner(new FileReader("piece-square_tables/player_bishop.txt"));
